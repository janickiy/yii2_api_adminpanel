...
## Вход в локальную админку

В текущем локальном Docker-окружении создана тестовая учётная запись:

| Логин | Пароль | Роль |
|---|---|---|
| `admin` | `1234567` | администратор |

Для входа используйте `admin / 1234567`.

Эти данные предназначены только для локальной разработки. Не используйте пароль `1234567` на публичном или production-сервере.

Миграции не записывают пароли и не создают администратора автоматически. После развёртывания чистой базы создайте первого администратора с собственным паролем длиной не менее 8 символов:
...
Публичные POST-запросы регистрации, входа в API и админку, а также формы обратной связи ограничены по IP на уровне Nginx. При превышении лимита возвращается HTTP `429` в JSON-формате с заголовками `Retry-After` и `X-Request-ID`.
...
Проект использует простую Yii2-ориентированную структуру. Общий код расположен в четырёх каталогах:

```text
common/
├── dtos/          простые объекты передачи валидированных данных
├── entities/      ActiveRecord-модели таблиц, правила БД и relations
├── repositories/  интерфейсы и работа с ActiveRecord, запросами и кэшем
└── services/      сценарии приложения, JWT, пароли и журналирование
```

Основной поток выполнения одинаков для REST API, внешней формы и административной панели:

```text
Controller → Form/Input → DTO → Service → Repository → ActiveRecord
```

- Controller принимает HTTP-запрос и выбирает ответ или представление.
- Yii Form/Input-модель нормализует и валидирует внешние данные.
...
В Docker по умолчанию `CACHE_DRIVER=memcached`, а время жизни запросов заметок задаётся `NOTES_CACHE_TTL`. Репозиторий кэширует отдельные заметки, списки и количество записей. После создания, изменения или удаления кэш пользователя инвалидируется. Ключи дополнительно содержат поколение кэша: старый запрос, завершившийся одновременно с записью в БД, не сможет повторно сохранить устаревшие данные.
...
Обычные ошибки Yii находятся в `runtime/logs` соответствующего приложения. Для внутренних ошибок API сохраняются stack trace и корреляционный `request_id`, который также возвращается клиенту в `X-Request-ID`. Логи контейнеров доступны через:
